...
import com.adtsw.jos.utils.LexicalAnalyser;
...
public class ScriptCompiler {
    ...
    private void compileScriptLine(ScriptLineCompilationContext compilationContext) {
        ...
        ScriptLineContext currentLineContext = setLineContext(lineNumber, line);
    ...
    private ScriptLineContext setLineContext(int lineNumber, String line) {
        ScriptLineContext lineContext = new ScriptLineContext();
        lineContext.setLineNumber(lineNumber);
        lineContext.setLine(line);
        lineContext.setLineType(getScriptLineType(line));
        return lineContext;
    }
    ...
    private ScriptLineObjectsContext getObjectPair(ScriptLineCompilationContext compilationContext, String line) {
        ...
        if(assignmentFunctionCallPattern.matcher(line).matches() || expressionPattern.matcher(line).matches() ||
...
//            case "IF":
//            case "LOG":
//            case "SETSTRATEGYACTION":
//            case "GETCUSTOMANALYSISVALUE":
//            case "SETCUSTOMANALYSISVALUE":
//            case "GETANALYSISVALUE":
//            case "ISTIMEBETWEEN":
//                lineContext.setExecutionPhase(ScriptLineExecutionPhase.RUN_TIME);
//                break;
//            case "ANALYSIS":
//            case "STRATEGY":
//            case "CUSTOMANALYSIS":
//                lineContext.setExecutionPhase(ScriptLineExecutionPhase.COMPILE_TIME);
//                break;
        }
        lineContext.setExecutionPhase(ScriptLineExecutionPhase.RUN_TIME);
    }
    ...
    private ScriptLineType getScriptLineType(String objectValue) {
        ...
        } else if(alphaNumericPattern.matcher(objectValue).matches()) {
            lineType = ScriptLineType.VALUE;
        } else if(expressionPattern.matcher(objectValue).matches()) {
            lineType = ScriptLineType.EXPRESSION;
        } else if(functionCallPattern.matcher(objectValue).matches()) {
